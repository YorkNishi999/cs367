...
public class WordCloudGenerator {
    ...
    /**
     * Generates an html file using the given list of words.  The html file
	...
	// Should the generated HTML be a simple alphabetic listing of the selected words
	...
	// With simpleDisplay = false, a more attractive two dimensional cloud will be generated
	//  with popular words in the centre and less popular words at the edges. The layout is
	...
	ArrayList<KeyWord2> cloudList = new ArrayList<KeyWord2>();
		...
		int numRows = (numWords%targetRowSize == 0)
			?  numWords/targetRowSize : numWords/targetRowSize + 1;
		if (debug)  System.out.println("Number of rows = " + numRows);
		...
		if (debug)  System.out.println(smallSizeCloudList);
		...
		for (int i = 1; i <= numRows; i++){
			int desiredSize = 
				targetRowSize - leftTable[i].size()  - rightTable[i].size();
			if (desiredSize <= availableCapacity)
				capacity[i] = desiredSize;
			else	capacity[i] = availableCapacity;
    ...
    private static void generateHtmlTable(List<KeyWord2> words, PrintStream out,
			 Pos position) {
	switch(position){
		case LEFT:
...
/**
 * A KeyWord2 is essentially a KeyWord, with ordering defined by the occur field rather than
 * the word field.
 */
class KeyWord2 implements Comparable<KeyWord2> {
    ...
    /**
     * Compares the KeyWord2 with the one given.  Two KeyWord2s are compared 
     * by comparing the occur filed associated with the two KeyWord2s.
     * @param other the KeyWord2 with which to compare this KeyWord2
     */
